...
export interface Version {version:number;createdAt:string;note:string;nodes:FlowNode[];edges:FlowEdge[];sourceVersion?:number}
export interface DraftWorkspace {nodes:FlowNode[];edges:FlowEdge[];restoredFromVersion?:number;updatedAt:string}
export interface Workflow {id:string;name:string;domain:string;status:WorkflowStatus;version:number;editor:string;updatedAt:string;publishedAt?:string;abnormalCount:number;nodes:FlowNode[];edges:FlowEdge[];versions:Version[];hasDraft?:boolean;draft?:DraftWorkspace|null}
